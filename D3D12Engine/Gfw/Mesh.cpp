...
void CTriangleMesh::CreateVertexBuffer(ID3D12Device* pDevice, ID3D12GraphicsCommandList* pCommandList)
{
	...
	pVertices[0] = CDiffusedVertex(Direction::UP, DirectX::XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f));
	pVertices[1] = CDiffusedVertex(Direction::RIGTH, DirectX::XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f));
	pVertices[2] = CDiffusedVertex(Direction::LEFT, DirectX::XMFLOAT4(0.0f, 0.0f, 1.0f, 1.0f));
...
void CCubeMesh::CreateVertexBuffer(ID3D12Device* pDevice, ID3D12GraphicsCommandList* pCommandList)
{
	...
	CVertex pVertices[8] = {
		CVertex(DirectX::XMFLOAT3(-0.5f, +0.5f, -0.5f)), // 0: 좌상앞 (빨강)
		CVertex(DirectX::XMFLOAT3(+0.5f, +0.5f, -0.5f)), // 1: 우상앞 (초록)
		CVertex(DirectX::XMFLOAT3(-0.5f, -0.5f, -0.5f)), // 2: 좌하앞 (파랑)
		CVertex(DirectX::XMFLOAT3(+0.5f, -0.5f, -0.5f)), // 3: 우하앞 (노랑)
		CVertex(DirectX::XMFLOAT3(-0.5f, +0.5f, +0.5f)), // 4: 좌상뒤 (자홍)
		CVertex(DirectX::XMFLOAT3(+0.5f, +0.5f, +0.5f)), // 5: 우상뒤 (청록)
		CVertex(DirectX::XMFLOAT3(-0.5f, -0.5f, +0.5f)), // 6: 좌하뒤 (흰색)
		CVertex(DirectX::XMFLOAT3(+0.5f, -0.5f, +0.5f)), // 7: 우하뒤 (검정)
	};
